...
export const SessionProvider: ParentComponent = (props) => {
  ...
  function handleMessageCreated(message: Message) {
    ...
    if (message.role === "assistant") {
      const msg = message as Message & { modelID?: string; finish?: string }
      if (msg.modelID && msg.finish) {
        const sel = selected()
        if (sel && sel.providerID === "kilo" && sel.modelID !== "OrqentAI/Auto" && msg.modelID !== sel.modelID) {
          showToast({
            variant: "default",
            title: "Model switched",
            description: `"${sel.modelID}" is rate-limited. Using "${msg.modelID}" instead.`,
            duration: 8000,
          })
          selectModel("kilo", msg.modelID)
        }
      }
    }
    // kilocode_change end
