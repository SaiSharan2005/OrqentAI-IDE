...
export const KILO_AUTO_SMALL_IDS = new Set(["kilo-auto/small", "auto-small", "OrqentAI/Auto-small"])
...
/** Friendly display names for well-known OrqentAI auto-resolution models. */
const AUTO_DISPLAY_NAMES: Record<string, string> = {
  "OrqentAI/Auto": "Auto",
  "OrqentAI/Auto-free": "Auto (Free)",
  "OrqentAI/Auto-small": "Auto (Small)",
}
...
export function buildTriggerLabel(
  ...
  if (raw?.providerID && raw?.modelID) {
    if (raw.providerID === KILO_GATEWAY_ID) {
      return AUTO_DISPLAY_NAMES[raw.modelID] ?? raw.modelID
    }
